    ...
    public partial class bot : Form
    {
        ...
        public bot()
        {
            ...
            async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
            {
                ...
                else if (messLow.StartsWith("khach"))
                {
                    reply = HoiData.ShowAllKh();
                }
                else if (messLow.StartsWith("qq "))
                {
                    //string tenKH = messageText.Substring(3);
                    //reply = HoiData.baoMotKhachHang();
                }
                else if (messLow.StartsWith("ten "))
                {
                    string tenKh = messageText.Substring(4); // Assuming "ten " is the prefix to remove
                    reply = HoiData.SearchKhachHangByTen(tenKh);
                }
